...
# Page configuration
st.set_page_config(page_title="Side by Side Comparison Report", layout="wide")
...
st.title("Side by Side Comparison Report Generator")
...
with col1:
    ...
    if not source_file:
        st.info("Using sample source data")
        source_df = pd.read_csv("sample_data/source.csv")
    else:
...
with col2:
    ...
    if not target_file:
        st.info("Using sample target data")
        target_df = pd.read_csv("sample_data/target.csv")
    else:
        target_df = pd.read_csv(target_file)
    st.dataframe(target_df.head(), use_container_width=True)

# Join column selection
st.subheader("Select Join Columns")
common_columns = list(set(source_df.columns) & set(target_df.columns))
join_columns = st.multiselect(
    "Select columns to use for matching records between source and target",
    ...
    if not join_columns:
        ...
        try:
            with st.spinner("Generating comparison report..."):
                ...
                report_path = rg.generate_side_by_side_report(source_df, target_df, join_columns)
                
                # Create zip archive
                zip_path = rg.create_report_archive({"SideBySideReport": report_path})
                ...
                # Add download button with clear styling
                st.download_button(
                    label="📥 Download Report (ZIP)",
                    data=zip_data,
                    file_name=Path(zip_path).name,
                    mime="application/zip",
                    help="Download the generated report in ZIP format"
                )
