...
class ComparisonEngine:
    ...
    def auto_map_columns(self) -> List[Dict[str, Any]]:
        ...
        # Create normalized lookup dictionaries for both source and target
        source_normalized = {normalize_for_matching(col): col for col in source_cols}
        target_normalized = {normalize_for_matching(col): col for col in target_cols}
        ...
        for s_col in source_cols:
            ...
            # If still no match, try normalized matching (handles spaces like 'mfa description')
            if not t_col:
                s_normalized = normalize_for_matching(s_col)
                # Check if normalized source exists in normalized target
                if s_normalized in target_normalized:
                    t_col = target_normalized[s_normalized]
                    # Log successful space-normalized matching for debugging
                    logger.info(f"Matched source '{s_col}' to target '{t_col}' using normalized matching")
        ...
        # But first check if they might match a source column through normalization
        for t_col in target_cols:
            if t_col not in mapped_targets:
                # Try to find a matching source column through normalization
                t_normalized = normalize_for_matching(t_col)
                matching_source = None
                
                # Check if this target column could match an unmapped source column
                for s_col in source_cols:
                    if s_col not in mapped_sources:
                        s_normalized = normalize_for_matching(s_col)
                        if s_normalized == t_normalized:
                            matching_source = s_col
                            logger.info(f"Found potential match: target '{t_col}' could map to unmapped source '{s_col}'")
                            break
                
                # Create mapping entry
                if matching_source:
                    # This target column has a potential source match that wasn't mapped earlier
                    source_type = str(self.source_df[matching_source].dtype)
                    target_type = str(self.target_df[t_col].dtype)
                    
                    # Apply same type mapping logic
                    if source_type == 'object':
                        source_type = 'string'
                    if target_type == 'object':
                        target_type = 'string'
                    ...
                    mapping_entry = {
                        ...
                        'target': t_col,
                        'join': False,
                        'data_type': 'string',  # Default to string for safety
                        'exclude': False,
                    ...
                    mapped_sources.add(matching_source)
                    mapped_targets.add(t_col)
                    logger.info(f"Added bidirectional mapping: '{matching_source}' <-> '{t_col}'")
                else:
                    # No matching source found, add as unmapped
                    mapping_entry = {
                        'source': '',
                        'target': t_col,
                        'join': False,
    ...
    def _prepare_dataframes(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for mapping in self.mapping:
            ...
            if source_col != target_col:
                logger.debug(f"Mapping source column '{source_col}' to target column '{target_col}'")
